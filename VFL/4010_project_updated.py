import secrets, hashlib

#扩展欧几里得算法
# Extended Euclidean Algorithm
def xgcd(a,b):
    prevx, x = 1, 0;  prevy, y = 0, 1
    while b:
        q, r = divmod(a,b)
        x, prevx = prevx - q*x, x
        y, prevy = prevy - q*y, y
        a, b = b, r
    return a, prevx, prevy

# Key Generation for Grab and Gojek
def keygen(p):
    while True:
        e = secrets.randbelow(p) #Encryption key  # Return a random int in the range [0, n).
        gcd, x, y = xgcd(e, p-1)
        if gcd != 1: continue
        d = x % (p-1)  #Decryption key
        break
    return e,d

# Encrypt m using key e in modulo p
def enc(e,m,p):
    return pow(m, e, p)

# Encrypt over a list
def enc_list(k, messages, p):
    result = set()
    for msg in messages:
        result.add(enc(k, msg, p))
    return result

# Hash of the given string of messages
def sha256(messages):
    result = set()
    for msg in messages:
        t= hashlib.sha256(msg.encode('utf-8')).hexdigest()
        result.add(int(t,base=16))
    return result

# Modulo p is public, Hexadecimal representation
p = 

# Start of code
grab_messages = {"91234567", "12345678","40000000", "52134567", "11112222", "98765432", "95678000"}
grab_messages_digest = sha256(grab_messages)  #对消息进行哈希处理

gojek_messages = {"95678000", "40000000", "68991111" "32123212", "52134567", "98765432"}
gojek_messages_digest = sha256(gojek_messages)  #对消息进行加密处理

# Private to Grab
x = keygen(p)[0]
grab_ciphertexts = enc_list(x, grab_messages_digest, p)  #先本地加密一波

# Private to Gojek
y = keygen(p)[0]
gojek_ciphertexts = enc_list(y, gojek_messages_digest, p) #先本地加密一波

# Encrypt with own private key
grab_double_enc = enc_list(x, gojek_ciphertexts, p)
gojek_double_enc = enc_list(y, grab_ciphertexts, p)

# Returns the number of drivers who are double-dipping
num = len(gojek_double_enc.intersection(grab_double_enc))
print("Number of intersections:", num)

# # Returns the set of doubly encrypted intersection numbers
# numlist = (gojek_double_enc.intersection(grab_double_enc))
# print("Intersection list", numlist)
doubledip = gojek_double_enc.intersection(grab_double_enc)  #返回两个里面都有的


# Getting the plaintext which has intersection
def intersect(messages, centre):
    result = set()
    for msg in messages:
        t = hashlib.sha256(msg.encode('utf-8')).hexdigest()
        u = int(t,base=16)
        v = enc(x, u, p)
        w = enc(y, v, p)
        if ( str(w) in str(centre)):
            result.add(msg)
    return result

double_dipper = intersect(grab_messages, doubledip)
print("\nPhone numbers:", double_dipper)